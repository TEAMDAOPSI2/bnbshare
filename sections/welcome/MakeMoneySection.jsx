      ...
      <div className="container py-8 px-3 mx-auto">
        <ReactWOW animation="slideInUp" duration="1s">
          <div className="text-center">
            <h2 className="text-4xl py-3 font-semibold">{t('how_we_make_money')}</h2>
            <p className="font-medium">{t('how_we_make_money_sub')}</p>
          </div>
        </ReactWOW>
        <div className="flex flex-col sm:flex-row gap-4 mt-6 justify-center items-center">
          <ReactWOW animation="slideInUp" duration="1s">
            <div className="info max-w-full sm:max-w-[300px] h-fit bg-white p-3 shadow rounded order-1">
              ...
              <h4 className="text-2xl font-medium py-3 w-[180px]">{t('monthly_rental_income')}</h4>
              <p>{t('monthly_rental_income_entry')}</p>
            </div>
          </ReactWOW>
          <ReactWOW animation="slideInUp" duration="1s">
            <div className="relative rounded shadow order-last sm:order-2 mx-10 sm:mx-0">
              <div className="flex flex-row h-[50px] w-fit py-3 px-5 rounded shadow justify-center bg-white absolute -left-10 top-2">
                ...
                <span className="flex items-center font-medium">{t('rented')}</span>
              </div>
              <img
                src={MakeMoneyPropertiesImg.src}
                className="w-[380px] h-[450px] object-cover rounded"
                alt="Make money properties"
              />
            </div>
          </ReactWOW>
          <ReactWOW animation="slideInUp" duration="1s">
            <div className="info max-w-full sm:max-w-[300px] h-fit bg-white p-3 shadow rounded order-3">
              ...
              <h4 className="text-2xl font-medium py-3 w-[180px]">{t('long_term_appreciation')}</h4>
              <p>{t('long_term_appreciation_entry')}</p>
            </div>
          </ReactWOW>
        </div>
      </div>
